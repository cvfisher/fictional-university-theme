...
class Search {
  ...
  // 2. events
  events() {
    this.openButton.forEach(button => button.addEventListener("click", () => this.openOverlay()));
  ...
  async getResults() {
    try {
      ...
      const [postsResponse, pagesResponse] = await Promise.all([fetch(`${universityData.root_url}/wp-json/wp/v2/posts?search=${searchTerm}`), fetch(`${universityData.root_url}/wp-json/wp/v2/pages?search=${searchTerm}`)]);
      ...
      ${combinedResults.map(item => `<li><a href="${item.link}">${item.title.rendered}</a>${item.type === "post" ? ` by <strong>${item.authorName}</strong>` : ""}</li>`).join("")}
  ...
  keyPressDispatcher(e) {
    const noInputFocused = !["INPUT", "TEXTAREA"].includes(document.activeElement.tagName);
